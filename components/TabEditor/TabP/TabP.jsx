...
class TabP extends Component {
  ...
  getVerticalLinesData = (delimiterIndexes, length) => {
    ...
    for (let i = 0; i < indexesLen; i++) {
      x =
        container.startX +
        (container.endX - container.startX) * (delimiterIndexes[i] / length);
      ...
      for (let j = 0; j < indexesLen; j++) {
        ...
        while ((matchResult = numRegExp.exec(bar))) {
          numberData.push({
            x:
              container.startX +
              ((preIndex + matchResult.index) / strings[i].length) *
                (container.endX - container.startX),
  ...
  parse = (source) => {
    ...
    verticalLinesData = this.getVerticalLinesData(
      delimiterIndexes,
      strings[0].length
    );
